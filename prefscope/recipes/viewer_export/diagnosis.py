...
from .sanitize import _clean_label, _label_or_id
...
def export_diagnosis(lens: Path, features: pd.DataFrame, min_battles=20, *,
    ...
    if not (bank / "bank_codes.npy").exists():
        return None
    from prefscope.recipes.pipeline.oriented_bank import load_bank
    ...
    if "concept" in features.columns:
        cmap = features.drop_duplicates("feature_id")
        resp_names = {
            int(row.feature_id): label
            for row in cmap.itertuples()
            if (label := _clean_label(row.concept)) is not None
        }
    else:
        resp_names = {}
    safe_prompt_names = prompt_names
    if isinstance(prompt_names, pd.DataFrame) and {
        "feature_id", "concept"
    } <= set(prompt_names.columns):
        safe_prompt_names = prompt_names.copy()
        safe_prompt_names["concept"] = [
            _label_or_id(value, feature_id, prefix="prompt feature")
            for value, feature_id in zip(
                safe_prompt_names["concept"], safe_prompt_names["feature_id"]
            )
        ]
    ...
    for m in keep_models:
        ...
        if prompt_wr_fn is not None:
            try:
                pw = prompt_wr_fn(prompt_lens, meta.loc[mask, bid_col].tolist(),
                                  win[mask], prompt_names=safe_prompt_names,
        ...
        if rel_fn is not None:
            try:
                rl = rel_fn(prompt_lens, meta.loc[mask, bid_col].tolist(),
                            cols[mask], feats, win[mask], prompt_names=safe_prompt_names,
        ...
        rows[m] = model_row
    concept_values = (features.set_index("feature_id").reindex(feats)["concept"].tolist()
                      if "concept" in features else [None] * len(feats))
    concepts = [_label_or_id(value, feature_id)
                for feature_id, value in zip(feats, concept_values)]
...
def export_head_to_head(lens: Path, features: pd.DataFrame, diag: dict | None,
    ...
    *same* nonzero mask on both sides, so a discordant count off the bank is always zero.
    ...
    concepts = diag.get("concepts") or [None] * len(feats)
    pairs = []
    for p in range(P):
    ...
    return {"models": models, "features": feats,
            "concepts": [_label_or_id(c, f) for f, c in zip(feats, concepts)],
            "presence_basis": ["semantic_threshold" if bool(v) else "positive_nonzero"
                               for v in calibrated],
