...
from .sanitize import _clean_label, _concept_or_id
...
def _visible_features(features: pd.DataFrame | None, width: int):
    ...
    if "concept" in table.columns:
        table = table[table["concept"].map(_clean_label).notna()]
    selection = "named"
...
def export_concept_distribution(lens, features: pd.DataFrame, *, chunk_rows: int = 50_000):
    ...
    if features is not None and {"feature_id", "concept"} <= set(features.columns):
        names = {int(r.feature_id): r.concept for r in features.itertuples()}
    for row in payload["features"]:
        row["concept"] = _concept_or_id(names, row["feature_id"])
    return payload
...
def export_prompt_concept_distribution(prompt_lens, features: pd.DataFrame, *,
    ...
    if features is not None and {"feature_id", "concept"} <= set(features.columns):
        names = {int(r.feature_id): r.concept for r in features.itertuples()}
    for row in payload["features"]:
        row["concept"] = _concept_or_id(
            names, row["feature_id"], prefix="prompt feature")
    return payload
...
def export_coactivation(lens, features: pd.DataFrame, *, top_k: int = 20,
    ...
    if features is not None and {"feature_id", "concept"} <= set(features.columns):
        ...
        for pair in payload["pairs"]:
            pair["a_concept"] = _concept_or_id(names, pair["a"])
            pair["b_concept"] = _concept_or_id(names, pair["b"])
    return payload
...
def export_prompt_coactivation(prompt_lens, features: pd.DataFrame, *,
    ...
    if features is not None and {"feature_id", "concept"} <= set(features.columns):
        names = {int(r.feature_id): r.concept for r in features.itertuples()}
        for pair in payload["pairs"]:
            pair["a_concept"] = _concept_or_id(
                names, pair["a"], prefix="prompt feature")
            pair["b_concept"] = _concept_or_id(
                names, pair["b"], prefix="prompt feature")
    return payload
