...
from .sanitize import _concept_or_id
...
def export_examples(lens: Path, corpus_path: str, features: pd.DataFrame,
    ...
    for feature_pos, f in enumerate(feats):
        ...
        # A single-response feature name describes the positive pole. Negative values
        # are not examples of that concept, even when their magnitude is large.
        picks = [
            int(i) for i in order
            if (col[i] != 0 if contrast else col[i] > 0)
        ][:n_per]
...
def export_report_battles(lens: Path, corpus_path: str, prompt_lens, diag,
    ...
    bid_concept = {
        b: (_concept_or_id(nmap, int(dom[i]), prefix="prompt feature")
            if dom[i] >= 0 else None)
...
def export_examples_by_model(lens: Path, corpus_path: str, features: pd.DataFrame,
    ...
    for r in top.itertuples():
        ...
        categorical = outcome(side, i)
        if categorical != "?":
            item["outcome"] = categorical
        if y is not None and not np.isnan(y[i]):
            probability = float(y[i]) if side == "a" else 1.0 - float(y[i])
            item["preference_probability"] = round(probability, 8)
        out.setdefault(r.m, {}).setdefault(str(int(r.f)), []).append(item)
    return out or None
