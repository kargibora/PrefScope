"""Export inspectable statistical feature communities for the external visualization clients."""
...
from .sanitize import _clean_label, _concept_or_id, _label_or_id, _read_csv, _round, _sanitize
...
def _meaningful_label(value, cluster_id: int) -> str | None:
    ...
    if label is None or re.fullmatch(r"cluster\s+\d+", label, flags=re.IGNORECASE):
        return None
...
def export_feature_clusters(
    ...
    *,
    ...
    for cluster_id, group in joined.groupby("cluster_id", sort=True):
        ...
        rep_concepts = _clean_label(summary_row.get("representative_concepts"))
        concept_map = dict(zip(group["feature_id"].astype(int), group.get(
            "concept", pd.Series(index=group.index, dtype=object))))
        if rep_concepts is None:
            label_prefix = "prompt feature" if kind == "prompt" else "feature"
            rep_concepts = " | ".join(
                _concept_or_id(concept_map, fid, prefix=label_prefix)
        ...
        member_table = group[member_columns].sort_values("feature_id").copy()
        if "concept" in member_table.columns:
            label_prefix = "prompt feature" if kind == "prompt" else "feature"
            member_table["concept"] = [
                _label_or_id(value, feature_id, prefix=label_prefix)
                for value, feature_id in zip(
                    member_table["concept"], member_table["feature_id"]
                )
            ]
        community = {
            "cluster_id": cid,
