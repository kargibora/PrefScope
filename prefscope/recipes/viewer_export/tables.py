...
from .sanitize import _concept_or_id, _label_or_id, _read_csv, _round
...
def export_delta(delta_csv, features: pd.DataFrame, bias_csv=None) -> dict | None:
    ...
    def _col(fid: int) -> dict:
        ...
        return {"id": int(fid), "concept": _label_or_id(g("concept"), int(fid)),
                "behavior": g("behavior"),
    ...
    return {"prompt_concepts": [{"id": int(k), "name": _concept_or_id(pc_name, k, prefix="prompt feature")} for k in pcs],
...
def export_conditional(cond_csv, features: pd.DataFrame, delta_csv=None) -> dict | None:
    ...
    return {"prompt_concepts": [{"id": k, "name": _concept_or_id(pc_name, k, prefix="prompt feature")} for k in pcs],
            "features": [{"id": f, "concept": _concept_or_id(cmap, f)} for f in fids],
            "cells": cells, "n_cells": len(cells), "n_significant": n_sig}
...
def export_elicitation(elic_csv, *, max_edges: int = 24000,
    ...
    return {"prompt_concepts": [{"id": k, "concept": _concept_or_id(pn, k, prefix="prompt feature")} for k in pxs],
            "response_concepts": [{"id": k, "concept": _concept_or_id(cn, k)} for k in cys],
            "edges": edges,
