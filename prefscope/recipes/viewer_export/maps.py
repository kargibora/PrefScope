...
from .sanitize import _clean_label, _concept_or_id, _concept_or_none, _read_csv
...
def export_feature_map(lens: Path, features: pd.DataFrame, *, seed: int = 0,
    ...
    def _named(fid: int) -> bool:
        ...
        return _concept_or_none({fid: value}, fid) is not None
...
def export_map(lens: Path, corpus_path: str, features: pd.DataFrame, *,
    ...
    try:
        ...
        print("  (skipping map.json — install `umap-learn` to use this optional recipe)",
              file=sys.stderr)
        return None
    ...
    return {"n_total": int(n), "n_sampled": int(len(idx)), "metric": metric, "mode": mode,
            "features": verified,
            # Missing names fall back to their stable feature IDs.
            "concepts": [_concept_or_id(concept_by, f) for f in verified],
            "points": pts}
...
def export_prompt_map(prompt_lens, completion_lens, delta_csv, prompt_interpret_dir, *,
    ...
    if pclu is not None and "cluster_id" in pclu.columns:
        ...
        if "behavior" in pclu.columns:
            behavior_rows = pclu.groupby("cluster_id")["behavior"].first().items()
            behaviors = {
                int(cluster_id): label
                for cluster_id, value in behavior_rows
                if (label := _clean_label(value)) is not None
            }
    ...
    for k, i in enumerate(idx):
        ...
        for j in np.argsort(-zpi[cand]):
            ...
            if z <= 1e-9:
                break
            pf.append({"id": int(candA[j]),
                       "concept": _concept_or_id(
                           pnames, int(candA[j]), prefix="prompt feature"),
                       "z": round(z, 3)})
            if len(pf) >= 5:
                break
        ...
        for j in np.argsort(-np.abs(zdi)):
            ...
            cf.append({"id": int(j), "concept": _concept_or_id(cnames, int(j)),
                       "z": round(z, 3),
    ...
    out = {"n_total": int(n), "n_sampled": int(len(idx)), "mode": mode,
           "features": feats,
           "concepts": [_concept_or_id(pnames, c, prefix="prompt feature") for c in feats],
           "points": pts}
    if f2c:
...
def export_response_map(lens, corpus_path, features, *, sample: int = 2500,
    ...
    return {"n_total": int(n2), "n_sampled": int(len(idx)), "mode": mode,
            ...
            "concepts": [_concept_or_id(concept_by, f) for f in vset], "points": pts}
