...
from prefscope.recipes.analysis.text_features import encode_text
...
def extract_present_concepts(
    ...
    *,
    ...
    presence = concept_presence(
        values, lens.feature_table, feature_ids=feature_ids, policy=policy
    )
    ...
    *,
    ...
    if question_lens is not None:
        result["prompt"] = extract_present_concepts(
            question_lens,
            encode_text(question_lens, prompt),
    ...
    if response_lens is not None:
        result["completion"] = extract_present_concepts(
            response_lens,
            encode_text(response_lens, prompt, completion),
            policy=presence_policy,
            fidelity_only=fidelity_only,
            top=top,
        )
