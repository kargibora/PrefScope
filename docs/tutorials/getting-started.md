...
PrefScope finds and compares concepts in post-training data. The next tutorial,
[Your first lens](your-first-lens.md), trains a lens. This tutorial only installs and
checks the software.
...
```bash
uv sync --extra arena    # + Hugging Face arena loaders (the build-corpus command)
...
GPU work needs a torch build matched to your accelerator. Install it using the official
PyTorch platform selector, then install/sync PrefScope. For portable CPU or Apple MPS:

```bash
uv sync --extra cpu     # CPU-only / CI
```

On macOS, `--extra cpu` resolves the regular PyPI torch wheel and therefore includes
MPS support. A plain `uv sync` remains torch-free on every platform.

### Keep the selected environment active

Extras are **not** automatically carried into a later `uv run`. Activate `.venv` after
syncing and call `prefscope` directly. For scripts
...
`prefscope init-demo` generates a tiny synthetic corpus from package code, and
`prefscope inspect` prints its summary. This works after a wheel installation: no clone,
model download, network, or API key is required.
...
You should see row and model counts, whether the data is paired, and the languages
present. Annotation JSON additionally reports the judge preference distribution. If you
do not have a table yet, this is
...
- A generated `demo/quickstart.yaml` and working `inspect` command — your fast,
  model-free sanity check on any battle
